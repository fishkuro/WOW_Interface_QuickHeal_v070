...
function QuickHeal_Shaman_GetRatioHealthyExplanation()
    ...
    if RatioHealthy >= RatioFull then
        return QUICKHEAL_SPELL_LESSER_HEALING_WAVE .. " 只在战斗中使用， "  .. QUICKHEAL_SPELL_HEALING_WAVE .. " 将在战斗外、战斗中且当治疗结果大于最大等级的" .. QUICKHEAL_SPELL_LESSER_HEALING_WAVE .. " 时使用. ";
    else
        if RatioHealthy > 0 then
            return QUICKHEAL_SPELL_LESSER_HEALING_WAVE .. " 只在战斗中使用， "  .. QUICKHEAL_SPELL_HEALING_WAVE .. " 会用于战斗中(如果目标生命超过" .. RatioHealthy*100 .. "%), 或者战斗外，或者当治疗结果大于最大等级的 " .. QUICKHEAL_SPELL_LESSER_HEALING_WAVE .. " 时. ";          
        else
            return QUICKHEAL_SPELL_LESSER_HEALING_WAVE .. " 永远不会被使用. " .. QUICKHEAL_SPELL_HEALING_WAVE .. " 将总是在战斗中和脱战后使用. "
        end
    end
end
...
--by Crazydru 增加BCS进行统计，kook区更新的BCS统计更准确
    ...
    if not forceMaxRank then
        ...
        if maxRankCH==1 then SpellID = SpellIDsCH[1]; HealSize = 356*hwMod+healMod25 end
        if maxRankCH==2 then SpellID = SpellIDsCH[2]; HealSize = 449*hwMod+healMod25 end
        if maxRankCH==3 then SpellID = SpellIDsCH[3]; HealSize = 607*hwMod+healMod25 end
...
--by Crazydru 增加BCS进行统计，kook区更新的BCS统计更准确
    ...
    local tfMod = 1 - talentRank/100;
    debug(string.format("Improved Healing modifier: %f", tfMod));
-- by Crazydru 自然迅捷天赋，下一个10s内法术瞬发
    local _,_,_,_,talentRank,_ = GetTalentInfo(3,13); 
    ...
    local maxRankLHW = table.getn(SpellIDsLHW);
    local NoLHW = maxRankLHW < 1;
--by Crazydru 自然迅捷技能读取
	local SpellIDsNS = nil
	local NSUsable=false
	if nsMod ==1 then
		SpellIDsNS = GetSpellIDs(QUICKHEAL_SPELL_NATURES_SWIFTNESS);
		NSUsable= GetSpellCooldown(SpellIDsNS, "spell")==0 and QuickHealVariables.Swiftness
	end
...
--by Crazydru 重构加血逻辑
    ...
    if not InCombat or TargetIsHealthy then
        -- Not in combat so use HW unless:
...
--by Crazydru
        ...
        if Health < RatioFull then
            ...
            if healneed > (292*hwMod+healMod30*PF18)*k and ManaLeft >= 155*tfMod and maxRankHW >=4 and downRankNH >=4 and SpellIDsHW[4] then SpellID = SpellIDsHW[4]; HealSize = 292*hwMod+healMod30*PF18 end
            ...
            if healneed > (408*hwMod+healMod30)*k and ManaLeft >= 200*tfMod and maxRankHW >=5 and downRankNH >=5 and SpellIDsHW[5] then SpellID = SpellIDsHW[5]; HealSize = 408*hwMod+healMod30 end
            ...
            if healneed > (579*hwMod+healMod30)*k and ManaLeft >= 265*tfMod and maxRankHW >=6 and downRankNH >=6 and SpellIDsHW[6] then SpellID = SpellIDsHW[6]; HealSize = 579*hwMod+healMod30 end
            ...
            if healneed > (797*hwMod+healMod30)*k and ManaLeft >= 340*tfMod and maxRankHW >=7 and downRankNH >=7 and SpellIDsHW[7] then SpellID = SpellIDsHW[7]; HealSize = 797*hwMod+healMod30 end
            ...
            if healneed > (1092*hwMod+healMod30)*k and ManaLeft >= 440*tfMod and maxRankHW >=8 and downRankNH >=8 and SpellIDsHW[8] then SpellID = SpellIDsHW[8]; HealSize = 1092*hwMod+healMod30 end
            if healneed > (1464*hwMod+healMod30)*k and ManaLeft >= 560*tfMod and maxRankHW >=9 and downRankNH >=9 and SpellIDsHW[9] then SpellID = SpellIDsHW[9]; HealSize = 1464*hwMod+healMod30 end
            if healneed > (1735*hwMod+healMod30)*k and ManaLeft >= 620*tfMod and maxRankHW >=10 and downRankNH >=10 and SpellIDsHW[10] then SpellID = SpellIDsHW[10]; HealSize = 1735*hwMod+healMod30 end
...
function QuickHeal_Command_Shaman(msg)
    ...
    writeLine(" [type] 治疗技能类型 [chainheal] or [heal]");
    writeLine(" [mod]  [chainheal]/[heal] 模式下额外选项:");
    writeLine("  [chainheal] 模式下额外选项:");
    writeLine("   [max] 用最高级治疗链治疗不满血目标");
    writeLine("  [heal] 模式下额外选项:");
    writeLine("   [max] 最高级治疗链治疗目标，无视是否掉血");
