...
--by Crazydru
--QuickHealVariables.Swiftmend = false;
--QuickHealVariables.Swiftness = false;
--QuickHealVariables.Waste = 50;
--QuickHealVariables.HolyShock = false
...
--by Crazydru 增加gcd检测判定是否施法忙
local function QuickHeal_Busy()
	local gcd = 0
	if type(GetNampowerVersion) ~= "nil" then
		local _, class = UnitClass("player")
		class = string.lower(class)
		if class == "druid" then
			gcd = GetSpellCooldown(QuickHeal_GetSpellInfo(QUICKHEAL_SPELL_HEALING_TOUCH)[1].SpellID, BOOKTYPE_SPELL)
		elseif class == "paladin" then
			gcd = GetSpellCooldown(QuickHeal_GetSpellInfo(QUICKHEAL_SPELL_HOLY_LIGHT)[1].SpellID, BOOKTYPE_SPELL)
		elseif class == "priest" then
			gcd = GetSpellCooldown(QuickHeal_GetSpellInfo(QUICKHEAL_SPELL_LESSER_HEAL)[1].SpellID, BOOKTYPE_SPELL)
		elseif class == "shaman" then
			gcd = GetSpellCooldown(QuickHeal_GetSpellInfo(QUICKHEAL_SPELL_HEALING_WAVE)[1].SpellID, BOOKTYPE_SPELL)
		end
		QuickHeal_debug("Check gcd done gcd:" .. gcd)
	end
	return gcd ~= 0 or QuickHealBusy
end


--增加黑名单查询功能（可搭配超级宏做判断目标是否黑名单）
function QH_CheckBlackList(unit)
	unit = unit or "target"
	local name = UnitFullName(unit)
	if not name then
		return
	end
	BlackList[name] = BlackList[name] or GetTime()-6
	return BlackList[name] > GetTime()
end

-- Returns true if unit has Renew buff
local function UnitHasRenew(unit)
    local BRenew = 'Interface\\Icons\\Spell_Holy_Renew'
    for j = 1, 40 do
        local B = UnitBuff(unit, j);
        if B then
            if B == BRenew or j==32 then
                return true
            end
        end
    end
...
local function UnitHasRejuvenation(unit) --
    local BRejuv = 'Interface\\Icons\\Spell_Nature_Rejuvenation'
    for j = 1, 40 do
        local B = UnitBuff(unit, j);
        if B then
            if B == BRejuv or j==32 then
                return true
            end
        end
    end
...
-- by Crazydru 当目标有hot的时候，返回true
function UnitHasHOT(unit)
	if PlayerClass == "priest" then
		return UnitHasRenew(unit)
	elseif PlayerClass == "druid" then
		return UnitHasRejuvenation(unit)
	end
	return false
end
...
--by Crazydru
        QuickHealConfig_CheckButtonSwiftmend:Hide();
        QuickHealConfig_Swiftmend:Hide();
        QuickHealVariables.Swiftness = true;
...
--by Crazydru
        QuickHealConfig_CheckButtonSwiftmend:Hide();
        QuickHealConfig_Swiftmend:Hide();
        QuickHealConfig_CheckButtonSwiftness:Hide();
        QuickHealConfig_Swiftness:Hide();
...
--by Crazydru
        ...
        QuickHealConfig_CheckButtonSwiftness:Hide();
        QuickHealConfig_Swiftness:Hide();
        --QuickHealVariables.HolyShock = true;
    ...
    elseif PlayerClass == "druid" then
        ...
        QuickHealDownrank_Slider_NH:SetValue(4)
        QuickHealDownrank_Slider_FH:SetMinMaxValues(1, 9);
        QuickHealDownrank_Slider_FH:SetValue(9)

--by Crazydru
        QuickHealVariables.Swiftmend = true;
        QuickHealVariables.Swiftness = true;
        --QuickHealConfig_CheckButtonHolyShock:Hide();
        --QuickHealConfig_HolyShock:Hide();
...
local function UpdateQuickHealOverhealStatus(multiplier)
    ...
    if waste > QuickHealVariables.Waste and HealMultiplier == 1.0 then
...
--by Crazydru 兼容Nampower
local function StopNampower()
	if type(GetNampowerVersion) ~= "nil" then 
		QuickHealVariables.NPQIS = GetCVar("NP_QueueInstantSpells")
		QuickHealVariables.NPQCTS = GetCVar("NP_QueueCastTimeSpells")
		QuickHealVariables.NPQTS = GetCVar("NP_QueueTargetingSpells")
		QuickHealVariables.NPQCS = GetCVar("NP_QueueChannelingSpells")
		QuickHealVariables.NPQOSS = GetCVar("NP_QueueOnSwingSpells")
		QuickHealVariables.NPQSOC = GetCVar("NP_QueueSpellsOnCooldown")
		...
		QuickHeal_debug("Nampower已停止")
	end
end
local function StartNampower()
	if type(GetNampowerVersion) ~= "nil" then 
		SetCVar("NP_QueueInstantSpells", QuickHealVariables.NPQIS) 
		SetCVar("NP_QueueCastTimeSpells", QuickHealVariables.NPQCTS) 
		SetCVar("NP_QueueTargetingSpells", QuickHealVariables.NPQTS)
		SetCVar("NP_QueueChannelingSpells", QuickHealVariables.NPQCS)
		SetCVar("NP_QueueOnSwingSpells", QuickHealVariables.NPQOSS)
		SetCVar("NP_QueueSpellsOnCooldown", QuickHealVariables.NPQSOC)
		QuickHeal_debug("Nampower已恢复")
	end
end
...
function NewUIErrorsFrame_OnEvent(...)
    ...
    if HealingTarget and event == "UI_ERROR_MESSAGE" and arg1 then
        if arg1 == ERR_SPELL_OUT_OF_RANGE and UnitFullName(HealingTarget) then
            Message(string.format(ERR_SPELL_OUT_OF_RANGE .. " %s 被列入黑名单5秒.", UnitFullName(HealingTarget)), "Blacklist", 5)
            LastBlackListTime = GetTime();
            BlackList[UnitFullName(HealingTarget)] = LastBlackListTime + 5;
            StopMonitor(arg1);
            return ;
        elseif arg1 == SPELL_FAILED_LINE_OF_SIGHT and UnitFullName(HealingTarget) then
...
function QuickHeal_GetExplanation(Parameter)
    ...
    if Parameter == "NotificationChannel" then
        if QHV.NotificationChannel then
            if QHV.NotificationChannelName and (QHV.NotificationChannelName ~= "") then
                return "通知将传送到频道 '" .. QHV.NotificationChannelName .. "' if it exists.";
            else
    ...
    if Parameter == "NotificationRaid" then
        if QHV.NotificationRaid then
            return "在RAID中，通知将发送到RAID频道.";
        else
            return "在RAID中，通知将不会发送到RAID频道.";
        end
    end
...
--by Crazydru
function QuickHeal_SetRatioHealthy(value)
    local _,PlayerClass = UnitClass('player');
    if string.lower(PlayerClass) == "druid" then QHV.RatioHealthyDruid=value end
    if string.lower(PlayerClass) == "paladin" then QHV.RatioHealthyPaladin=value end
    if string.lower(PlayerClass) == "priest" then QHV.RatioHealthyPriest=value end
    if string.lower(PlayerClass) == "shaman" then QHV.RatioHealthyShaman=value end
    return nil;
end
--by Crazydru
function QuickHeal_SetDownrankValue(value1,value2)
	if value1 then
		QuickHealDownrank_Slider_NH:SetValue(value1)
	end
	if value2 then
		QuickHealDownrank_Slider_FH:SetValue(value2)
	end
end
--by Crazydru
function QuickHeal_SetWasteValue(value)
	if value then
		QuickHealVariables.Waste=value
	else 
		QuickHealVariables.Waste=30
	end
end
...
local function UnitIsHealable(unit, explain)
    if UnitExists(unit) then
        if EvaluateUnitCondition(unit, UnitIsFriend('player', unit), "非友方目标", explain) then
            return false
        end
        if EvaluateUnitCondition(unit, not UnitIsEnemy(unit, 'player'), "是敌人", explain) then
            return false
        end
        if EvaluateUnitCondition(unit, not UnitCanAttack('player', unit), "可以被玩家攻击", explain) then
            return false
        end
        if EvaluateUnitCondition(unit, UnitIsConnected(unit), "未连接", explain) then
            return false
        end
        if EvaluateUnitCondition(unit, not UnitIsDeadOrGhost(unit), "是死亡的或者灵魂", explain) then
            return false
        end
        if EvaluateUnitCondition(unit, UnitIsVisible(unit), "对客户端不可见", explain) then
            return false
        end
    else
        return false
...
local function HasRegrowthRank1()
    for i = 1, MAX_SPELLS do
        ...
        if spellName == QUICKHEAL_SPELL_REGROWTH then
...
-- by Crazydru
	StopNampower()
	QuickHeal_debug("********** BREAKPOINT: CastCheckSpell() **********")
    ...
    QuickHeal_debug("********** BREAKPOINT: CastCheckSpell() done **********");
-- by Crazydru
	StartNampower()
...
local function ExecuteHeal(Target, SpellID)
    ...
    if UnitIsHealable('target') then
        ...
        if Target == 'targettarget' then
            ...
            QuickHeal_debug("目标无法治疗，暂时将目标切换到目标的目标", old, '-->', UnitFullName('target'));
        end
        -- If healing target is not the current healable target clear the healable target
        if not (Target == 'target') then
            QuickHeal_debug("目标无法治疗，暂时清除目标", UnitFullName('target'));
            ClearTarget();
            TargetWasChanged = true;
        end
    end
        ...
        if UnitIsHealable(Target, true) then
            ...
            if targetPercentage < QHV.RatioFull then
                ...
                elseif Target == "targettarget" then
                    Message(UnitFullName('target') .. "的目标 (" .. UnitFullName('targettarget') .. ") 无需治疗", "Info", 2);
                else
                    Message(UnitFullName(Target) .. " 无需治疗", "Info", 2);
                end
                SetCVar("autoSelfCast", AutoSelfCast);
                QuickHealBusy = false;
                return ;
            ...
            elseif Target == 'targettarget' then
                ...
                elseif not UnitExists('targettarget') then
                    ...
                    Message(UnitFullName('target') .. "的目标 (" .. UnitFullName('targettarget') .. ") 不需要治疗", "Error", 2);
                end
            elseif UnitExists(Target) then
                -- Unit exists but cannot be healed
                if UnitIsUnit(Target, 'player') then
                    Message("你无法被治疗", "Error", 2);
                else
                    Message(UnitFullName(Target) .. " 不需要治疗", "Error", 2);
                end
            else
                Message("目标不存在", "Error", 2);
            end
            SetCVar("autoSelfCast", AutoSelfCast);
            QuickHealBusy = false;
            return ;
...
--by Crazydru 增加noHpCheck判断
            if (targetPercentage < QHV.RatioFull or noHpCheck) and not UnitHasHOT(Target) then
                -- Does need healing (fall through to healing code)
            else
                -- Does not need healing
                if UnitIsUnit(Target, 'player') then
...
function QH_FindWhoToHOT(Restrict)
	local AutoSelfCast = GetCVar("autoSelfCast")
	SetCVar("autoSelfCast", 0)
	local result = FindWhoToHOT(Restrict)
	SetCVar("autoSelfCast", AutoSelfCast)
	return result
end
